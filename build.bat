...
rem ============================================================
rem build.bat - Windows/MSVC + vcpkg manifest configure/build/run
rem - Avoids line-continuation (^) pitfalls that break quoting in cmd.exe
rem - Enables Vulkan validation layers for this run if available
rem ============================================================
...
echo Configuring...
cmake -S "%ROOT%" -B "%BUILD_DIR%" -G "Visual Studio 17 2022" -A x64 -DCMAKE_TOOLCHAIN_FILE:FILEPATH="%TOOLCHAIN%" -DVCPKG_TARGET_TRIPLET=%TRIPLET% -DVCPKG_FEATURE_FLAGS=manifests
...
rem Prefer VK_ADD_LAYER_PATH (newer loader) and also set VK_LAYER_PATH for compatibility.
if exist "%INST%\bin\VkLayer_khronos_validation.json" (
  ...
  echo WARNING: Validation layer json not found at "%INST%\bin\VkLayer_khronos_validation.json"
)
